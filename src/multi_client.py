...
class CustomPlayer(CTkFrame):
    ...
    def __init__(self, host, port, name, master):
    ...
    def recve(self):
        while True:
            ...
            data = loads(self.x.recv(4096))
            print(data)
            self.status = data[0]
            print("status:", self.status)
            if self.status == "W":
                self.master.switch_frame("PageWaiting")
                self.statusWaiting = self.statusWaiting
    ...
    def getStatusWaiting(self):
        return self.statusWaiting
